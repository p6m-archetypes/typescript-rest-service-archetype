...
// black-box test can drive. Rename alongside src/persistence/schema.ts when you
// add your real model. Writes generate ids in JS and read back after the write so
// the routes stay dialect-portable (MySQL has no RETURNING clause).
const itemRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.post<{ Body: ItemBody }>('/api/items', async (request, reply) => {
    const id = randomUUID();
  ...
  fastify.put<{ Body: ItemBody; Params: ItemParams }>('/api/items/:id', async (request, reply) => {
  ...
  fastify.delete<{ Params: ItemParams }>('/api/items/:id', async (request, reply) => {
    const { id } = request.params;
    const [existing] = await fastify.db.select().from(items).where(eq(items.id, id));
    if (!existing) return reply.code(404).send({ message: 'Not Found' });
